...
def main_worker():
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_noisy_set)
    ...
    train_loader = DataLoader(dataset=train_noisy_set, sampler=train_sampler, batch_size=args.batch_size // (2 * num_gpu),
    ...
    for epoch in range(args.start_epoch, args.end_epoch):
        ...
        for i, data in enumerate(train_loader):
            ...
            ind1 = random.randint(0, len(train_gt_set)-1)
            # ind2 = random.randint(0, len(train_gt_set))
            x_gt, target_gt = train_gt_set[ind1]
            x_gt = torch.unsqueeze(x_gt, 0)
            target_gt = torch.unsqueeze(target_gt, 0)
            x_cr, target_cr = data

            x = torch.cat((x_gt, x_cr), dim=0)
            target = torch.cat((target_gt, target_cr), dim=0)
            ...
            ##=========================================
            target[target == 4] = 3
            loss_gt =  torch.nn.CrossEntropyLoss()(flatten_func(output[:1,:,:,:,:]).permute((1, 0)), torch.flatten(target[:1]))
            loss_cr = criterion(flatten_func(output[1:,:,:,:,:]).permute((1, 0)), torch.flatten(target[1:]))
            loss = loss_cr + loss_gt
            ##==========================================

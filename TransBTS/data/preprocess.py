...
# train
train_set = {
        'root': '/ImagePTE1/ajoshi/data/BRATS2018/Training/',
        'flist': 'train_list.txt',
        'has_label': True
        }
...
def process_f32b0(path, has_label=True):
    ...
    # print(path)
    # pdb.set_trace()
    output_path = '/home/wenhuicu/brats_preprocessed/'
    sub_name = path.split('/')[-2]
    folder_dir = output_path + sub_name + '/'
    if os.path.exists(folder_dir) == False:
        os.mkdir(folder_dir)

    output = folder_dir + sub_name + '_data_f32b0.pkl'
    mask = images.sum(-1) > 0
    for k in range(4):
